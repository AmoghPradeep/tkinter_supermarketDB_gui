...
class createFrame:
    ...
    def createInsert(self):
        insert = LabelFrame(self.frame, text = "Insert", font = ("Montserrat", 16, 'italic'))
        ...
        for field in self.field_list:
            new_label = Label(insert, text = field + " : ",font = ("Montserrat", 12))
        ...
        sumbit = Button(insert, text = "Submit", bg = "orange", command = lambda: self.insertDB(textBoxes), font = "Montserrat").place(x = 250, y = init_y + 25, width = 200)
        init_y = 30
    ...
    def createDelete(self, primary_key):
        delete = LabelFrame(self.frame, text = "Delete", font = ("Montserrat", 16, 'italic'))
        delete.place(x = 550, y = 0, height = 320, width = 500)

        pk_label = Label(delete, text = primary_key + " : ", font =("Montserrat", 12))
        pk_label.place(x = 30, y = 30)

        textBox = Text(delete, bg = "#ffe0bf") 
        sumbit = Button(delete, text = "Submit", bg = "orange", command = lambda: self.delDB(textBox, primary_key), font = "Montserrat").place(x = 250, y = 75, width = 200)

        textBox.place(x = 150, y = 30, width = 300, height = 20)

    def createDisplay(self):
        display = LabelFrame(self.frame, text = "Table Data", font = ("Montserrat", 16, 'italic'))
        display.place(x = 20, y = 340, height = 350, width = 1030)
        column = StringVar()
        column.set("All")
        colSel = OptionMenu(display,column, *self.field_list)
        colSel.place(x = 275, y = 84, width = 200, height = 30)
        showcolumn = Label(display, text = "Select Column",font = ("Montserrat", 16)).place(x= 100, y = 80)
        selectCond = Label(display, text = "Condition",font = ("Montserrat", 16)).place(x = 525, y = 80)

        viewTableButton.viewTable(display, self.table, self.field_list, column)
